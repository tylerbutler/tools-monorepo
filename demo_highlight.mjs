...
import CCLModule from './bindings/node/index.js';
const CCL = CCLModule.default || CCLModule;
...
const colors = {
  ...
  error: pc.red,
  success: pc.green,
  heading: (text) => pc.yellow(pc.bold(text)),
  reset: pc.reset
};
...
function highlightCCL(text, tree) {
  ...
  lines.forEach((line, row) => {
    if (line.startsWith('#')) {
      ...
      const lineHighlights = highlights.get(row).sort((a, b) => a.startCol - b.startCol);
...
function testMultilineParsing() {
  ...
  testCases.forEach(test => {
    ...
    if (hasError) {
      console.log(`  ${colors.error('✗')} Failed to parse (contains ERROR nodes)`);
    } else if (hasMultiline) {
      console.log(`  ${colors.success('✓')} Correctly parsed as multiline value`);
    } else {
...
async function main() {
  ...
  // Check if picocolors is installed
  try {
    await import('picocolors');
  } catch(e) {
    console.log('Installing picocolors for terminal colors...');
    execSync('npm install picocolors', { stdio: 'inherit' });
  ...
  // Summary
  console.log(colors.heading('\n=== Summary ===\n'));
  console.log(`${colors.success('✓')} Multiline values now parse correctly (key = \\n  indented)`);
  console.log(`${colors.success('✓')} Comments are properly recognized (/= comment text)`);
  ...
  console.log(`${colors.error('✗')} Empty values still have issues (tree-sitter limitation)`);
