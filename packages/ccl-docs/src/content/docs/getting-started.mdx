...
These are treated as literal key strings (`"database.host"`, `"database.port"`, etc.) - no hierarchy is provided for dotted keys automatically. See [Dotted Keys FAQ](/ccl-faq#dotted-keys) for details.
:::
