/**
 * MouseTracker - tracks the mouse position of all connected sessions using
 ...
 *
 * - mousePositionChanged: Emitted any time the MouseTracker detects a change
 *   in the mouse position of any client, local or remote.
 */
export type IMouseTrackerEvents = {
	mousePositionChanged: [];
};

/**
 * The MouseTracker class tracks the mouse position of all connected sessions
 * using the Fluid Framework presence features. Mouse position is tracked
 * automatically by the class instance. As the mouse position of connected
 * sessions changes, the MouseTracker emits a "mousePositionChanged" event.
 */
export class MouseTracker extends TypedEventEmitter<IMouseTrackerEvents> {
	...
	public constructor(
		...
		/**
		 * A states workspace that the MouseTracker will use to share mouse
		 * positions with other session clients.
		 */
		...
		this.cursor = statesWorkspace.states.cursor as LatestRaw<IMousePosition>;

		// When the cursor state is updated, the MouseTracker should emit the
		// mousePositionChanged event.
		this.cursor.events.on("remoteUpdated", () => {
			this.emit("mousePositionChanged");
		});

		// When an attendee disconnects, emit the mousePositionChanged event so
		// client can update their rendered view accordingly.
		this.presence.attendees.events.on("attendeeDisconnected", () => {
			this.emit("mousePositionChanged");
		});

		// Listen to the local mousemove event and update the local position.
	 ...
	 *
	 * @param latency - the maximum allowable latency for updates.
	 *   Set to undefined to revert to the default value.
	 */
