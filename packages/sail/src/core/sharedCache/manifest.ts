/*!
 * Cache manifest operations for reading/writing cache metadata.
 *
 ...
 * @returns The parsed manifest, or undefined if invalid
 */
export async function readManifest(
	manifestPath: string,
): Promise<CacheManifest | undefined> {
	try {
...
function validateManifest(manifest: CacheManifest): void {
	...
	if (manifest.exitCode !== 0) {
		throw new Error(
			`Invalid exit code in manifest: ${manifest.exitCode} (only 0 is valid)`,
		);
	}

	// Validate execution time
	if (
		typeof manifest.executionTimeMs !== "number" ||
		manifest.executionTimeMs < 0
	) {
	...
	if (
		typeof manifest.cacheSchemaVersion !== "number" ||
		manifest.cacheSchemaVersion < 1
	) {
	...
	if (Number.isNaN(accessedDate.getTime())) {
		throw new Error(
			`Invalid lastAccessedAt timestamp: ${manifest.lastAccessedAt}`,
		);
	}
}
 ...
 * @param manifestPath - Path to the manifest.json file
 */
export async function updateManifestAccessTime(
	manifestPath: string,
): Promise<void> {
