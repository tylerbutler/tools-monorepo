...
// Tab width constant - tabs count as 8 spaces (standard terminal convention)
    ...
    if ((lexer->lookahead == '\n' || lexer->lookahead == '\r') && valid_symbols[DEDENT] && !valid_symbols[NEWLINE] && !at_line_start) {
      ...
      while (lexer->lookahead == ' ' || lexer->lookahead == '\t') {
        if (lexer->lookahead == ' ') {
          ...
          next_indent += TAB_WIDTH;  // Each tab = TAB_WIDTH indentation units
        }
    ...
    if (at_line_start) {
      ...
      while (lexer->lookahead == ' ' || lexer->lookahead == '\t') {
        if (lexer->lookahead == ' ') {
          ...
          current_indent += TAB_WIDTH;  // Each tab = TAB_WIDTH indentation units
        }
