...
interface PolicyInfo {
  ...
  filePatternReadable: string; // Human-readable pattern description
  configSummary?: string;    // Brief config summary (keys only)
  configDetails?: string;    // Full config JSON for display
  excludedFiles: string[];   // Patterns excluded from policy
}
```
...
| `showConfig` | `boolean` | `false` | Show config summary column (keys only) |
| `showFilePattern` | `boolean` | `true` | Show regex pattern column |
| `showReadablePattern` | `boolean` | `false` | Show human-readable "Matches" column |
| `showConfigDetails` | `boolean` | `false` | Show full config JSON in "Configuration" column |
| `showExcludedFiles` | `boolean` | `false` | Show excluded patterns in "Excluded" column |
