 ...
 */
export function findGitRootSync(cwd = process.cwd()) {
    try {
 ...
 */
export async function getMergeBaseRemote(git, branch, remote, localRef = "HEAD") {
    if (remote !== undefined) {
 ...
 */
export async function getChangedFilesSinceRef(git, ref, remote) {
    if (remote !== undefined) {
        // make sure we have the latest remote refs
        await git.fetch([remote]);
    }
 ...
 */
function filePathsToDirectories(files) {
 ...
 */
export async function getChangedSinceRef(buildProject, ref, remote) {
    ...
    const changedPackages = [...new Set(changedPackageNames)]
        .map((name) => buildProject.packages.get(name))
 ...
 */
export async function getRemote(git, partialUrl) {
...
export async function getFiles(git, directory) {
    ...
    const allFiles = new Set(results
        .split("\n")
        .map((line) => line.trim())
