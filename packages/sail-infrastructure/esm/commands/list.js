...
export class ListCommand extends Command {
    static description = "List objects in the build project, like release groups, workspaces, and packages. USED FOR TESTING ONLY.";
    static flags = {
        ...
        for (const workspace of repo.workspaces.values()) {
            ...
            for (const releaseGroup of workspace.releaseGroups.values()) {
                ...
                for (const pkg of releaseGroup.packages) {
                    const pkgMessage = colors.white(`${pkg.name}${pkg.isReleaseGroupRoot ? colors.bold(" (root)") : ""}`);
    ...
    async logCompactReport(repo) {
        ...
        for (const workspace of repo.workspaces.values()) {
            ...
            for (const pkg of workspace.packages) {
                const pkgMessage = colors.white(`${pkg.isReleaseGroupRoot ? colors.bold("(root) ") : ""}${pkg.name} ${colors.black(colors.bgGreen(pkg.releaseGroup))}`);
                this.logIndent(pkgMessage, 3);
            }
        }
    }
    logIndent(message, indent = 0) {
