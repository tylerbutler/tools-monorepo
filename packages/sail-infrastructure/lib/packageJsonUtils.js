 ...
 */
export function updatePackageJsonFile(packagePath, packageTransformer) {
    const resolvedPath = packagePath.endsWith("package.json")
        ? packagePath
        : path.join(packagePath, "package.json");
    const [pkgJson, indent] = readPackageJsonAndIndent(resolvedPath);
 ...
 * indentation.
 */
export function readPackageJsonAndIndent(pathToJson) {
 ...
 */
export function writePackageJson(packagePath, pkgJson, indent) {
 ...
 * The package.json is always sorted using sort-package-json.
 */
export async function updatePackageJsonFileAsync(packagePath, packageTransformer) {
    const resolvedPath = packagePath.endsWith("package.json")
        ? packagePath
        : path.join(packagePath, "package.json");
    const [pkgJson, indent] = await readPackageJsonAndIndentAsync(resolvedPath);
    // Transform the package.json
 ...
 * indentation.
 */
async function readPackageJsonAndIndentAsync(pathToJson) {
