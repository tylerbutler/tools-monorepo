...
export function buildFromDecomposition<TBTree extends BTree<K, V>, K, V>(
  ...
  const leftContext: SideContext<K, V> = {
    branchingFactor,
    spine: frontier,
    ...
    updateMax: noop, // left side appending doesn't update max keys,
...
function splitUpwardsAndInsert<K, V>(
  ...
  subtree: BNode<K, V>
): [newRoot: BNodeInternal<K, V> | undefined, insertTarget: BNodeInternal<K, V>] {
...
function splitUpwardsAndInsertEntries<K, V>(
  ...
  if (parentSize + entryCount <= branchingFactor) {
    ...
    const [newRoot, grandparent] = splitUpwardsAndInsert(
      context,
      insertionDepth - 1,
      entryContainer
    );
 ...
 */
function updateFrontier<K, V>(context: SideContext<K, V>, depthLastValid: number): void {
 ...
 */
function findSplitCascadeEndDepth<K, V>(context: SideContext<K, V>, insertionDepth: number, insertionCount: number): number {
...
function balanceLeavesRight<K,V>(parent: BNodeInternal<K,V>, underfilled: BNode<K, V>, toTake: number): void {
...
function balanceLeavesLeft<K,V>(parent: BNodeInternal<K,V>, underfilled: BNode<K, V>, toTake: number): void {
