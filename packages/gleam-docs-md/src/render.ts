/**
 * Pure rendering primitives that turn a parsed Gleam `package-interface.json`
 * into Markdown strings. These functions perform no I/O — callers (CLI,
 * Astro loader, Starlight plugin) handle reading the JSON and writing the
 * output.
 *
 * @remarks
...
/**
 * One rendered page produced by {@link renderPackage}.
 *
 * @beta
 */
export interface RenderedPage {
	/** Slug used as the filename stem (without extension). */
	slug: string;
	/** Source module name from the package interface (e.g. `beryl/channel`). */
	moduleName: string | undefined;
	/** Page title (the module name for module pages, "Reference" for index). */
	title: string;
	/** First-line description, suitable for frontmatter. */
	description: string;
...
/**
 * Options for {@link renderPackage}.
 *
 * @beta
 */
...
/**
 * Render an index page plus one page per module from a parsed package
 * interface.
 *
 * @beta
 */
export function renderPackage(
	...
	const linkPrefix = options.linkPrefix ?? "/reference/";
	...
	const pages: RenderedPage[] = [
		{
			...
			markdown: renderIndex(packageInterface, modules, linkPrefix),
		},
	];
	...
	for (const [moduleName, moduleInterface] of modules) {
		...
		pages.push({
			...
			title: moduleName,
			description,
			markdown: renderModulePage(moduleName, moduleInterface),
		});
	}
...
/**
 * Slugify a Gleam module name (`foo/bar/baz` → `foo-bar-baz`).
 *
 * @beta
 */
...
function renderIndex(
	...
	const moduleRows = modules
		.map(([moduleName, moduleInterface]) => {
			...
			return `| [${code(moduleName)}](${linkPrefix}${moduleSlug(moduleName)}/) | ${description} |`;
		})
		.join("\n");
...
Pages in this section are generated from Gleam's docs metadata and reflect every public type, function, and constant.
:::
