...
test.describe("Load Built-in Data Button", () => {
	...
	test("loads built-in data on first click and shows success message", async ({ page }) => {
		...
		const loadBuiltinButton = page.getByRole("button", { name: "Load Built-in Data" });
		...
		await expect(page.getByText("Loaded built-in data: 366 tests across 12 categories")).toBeVisible({
			timeout: 5000
		});
		...
		await expect(page.getByText("Loaded built-in data: 366 tests across 12 categories")).not.toBeVisible({
			timeout: 4000
		});
	});
	...
	test("multiple rapid clicks are handled gracefully", async ({ page }) => {
		...
		await expect(successMessage.or(alreadyLoadedMessage)).toBeVisible({ timeout: 5000 });
