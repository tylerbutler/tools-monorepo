...
test.describe('CCL Test Viewer App', () => {
	test('homepage loads successfully', async ({ page }) => {
		...
		await expect(page.getByRole('heading', { name: 'CCL Test Suite Viewer' })).toBeVisible();

		// Check for navigation links
		await expect(page.getByRole('button', { name: /home/i })).toBeVisible();
		await expect(page.getByRole('button', { name: /browse tests/i })).toBeVisible();
	});
	...
	test('browse page displays tests', async ({ page }) => {
		...
		await expect(page.getByText('Loading test data...')).not.toBeVisible({ timeout: 10000 });
	...
	test('filtering functionality works', async ({ page }) => {
		...
		const sidebar = page.getByRole('complementary', { name: 'Test filters and search' });
		...
		// Try to use search
		const searchInput = page.getByRole('searchbox', { name: 'Search through test cases' });
	...
	test('responsive design works on mobile', async ({ page }) => {
		...
		const mobileMenuToggle = page.getByRole('button', { name: /open filters|close filters/i });
	...
	test('view mode toggle works', async ({ page }) => {
		...
		const gridContainer = page.locator('div').filter({ hasText: /View test case/ }).first();
