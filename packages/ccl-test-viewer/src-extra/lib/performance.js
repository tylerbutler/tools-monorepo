...
export function withPerformanceTracking(componentName) {
    return {
        ...
        onDestroy() {
            ...
            collector?.measureCustom(`${componentName}Lifetime`, `component-${componentName}`, `component-${componentName}`);
        }
    };
}
...
export class PerformanceBudget {
    ...
    checkBudget(metricName, value) {
        ...
        if (budget && value > budget) {
            ...
            document.dispatchEvent(new CustomEvent('performance-budget-violation', {
                detail: violation
            }));
...
export const CCLPerformance = {
    ...
    trackSearch: (query, searchFn) => {
        const operation = () => {
            ...
            collector?.measureCustom('searchQueryTime', 'search-query', 'search-query');
            return result;
        };
    ...
    monitorDataOperation: (operationName, dataSize, operationFn) => {
        ...
        const operation = () => {
            ...
            if (dataSize > 1000 || duration > 100) {
                console.log(`📊 ${operationName}: ${duration.toFixed(1)}ms for ${dataSize} items`);
            }
