...
export type CCLFunction = ${Array.from(allFunctions).map(f => `"${f}"`).join(' | ')};
export type CCLFeature = ${Array.from(allFeatures).map(f => `"${f}"`).join(' | ')};
export type CCLBehavior = ${Array.from(allBehaviors).map(b => `"${b}"`).join(' | ')};

// Available functions for filtering
export const AVAILABLE_FUNCTIONS: CCLFunction[] = [${Array.from(allFunctions).map(f => `"${f}"`).join(', ')}];

// Available features for filtering
export const AVAILABLE_FEATURES: CCLFeature[] = [${Array.from(allFeatures).map(f => `"${f}"`).join(', ')}];

// Available behaviors for filtering
export const AVAILABLE_BEHAVIORS: CCLBehavior[] = [${Array.from(allBehaviors).map(b => `"${b}"`).join(', ')}];
`;
}
...
async function main(): Promise<void> {
  ...
  console.log(`🔍 Generated search index with ${Object.keys(searchIndex.byName).length} name tokens`);
  ...
  console.log(`📊 Summary: ${summary.stats.categories} categories, ${summary.stats.totalTests} tests, ${summary.stats.totalAssertions} assertions`);
}
