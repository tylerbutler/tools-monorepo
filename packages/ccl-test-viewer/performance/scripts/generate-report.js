...
class ReportGenerator {
  ...
  async collectData(timeRange) {
    ...
    const lighthouseHistoryPath = join(projectRoot, 'perf-data/lighthouse-reports/history.json');
    ...
    // Collect bundle data
    const bundleHistoryPath = join(projectRoot, 'perf-data/bundles/history.json');
  ...
  analyzeTrends(data) {
    return {
      ...
      bundleSize: this.calculateTrend(data.bundles, entry => entry.totals?.size),
  ...
  analyzePerformance(data) {
    ...
    const metrics = ['performance', 'accessibility', 'bestPractices', 'seo', 'lcp', 'cls', 'fid'];
    const analysis = {};

    for (const metric of metrics) {
      const values = lighthouse.map(entry => entry[metric]).filter(v => v !== undefined);
      if (values.length > 0) {
        analysis[metric] = {
  ...
  analyzeBundleSize(data) {
    ...
    const sizes = bundles.map(b => b.totals?.size).filter(s => s !== undefined);
    ...
    return {
      ...
      compressionRatio: latest.totals?.gzipSize && latest.totals?.size
  ...
  generateRecommendations(data) {
    ...
    recommendations.push({
      ...
      description: '436KB of test data could be optimized for better initial load performance.',
      suggestions: [
...
`;
  ...
  formatAsMarkdown(report) {
    const { summary, trends, performance, bundleAnalysis, recommendations } = report;
...
### Performance Metrics
${performance ? Object.entries(performance).map(([metric, data]) =>
...
#### Breakdown by Type
${Object.entries(bundleAnalysis.breakdown || {}).map(([type, data]) =>
