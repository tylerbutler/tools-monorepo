...
class BundleAnalyzer {
	...
	generateRecommendations(analysis) {
		...
		if (totals.size > 1024 * 1024) {
			...
			recommendations.push({
				...
				suggestion:
					"Consider code splitting and lazy loading for non-critical resources",
			});
		}
		...
		if (jsSize > 400 * 1024) {
			...
			recommendations.push({
				...
				suggestion:
					"Consider tree shaking, code splitting, and removing unused dependencies",
			});
		}

		// Static data recommendations
		if (staticData.totalSize > 500 * 1024) {
			// 500KB
			recommendations.push({
				type: "info",
				...
				suggestion:
					"Consider data pagination, lazy loading, or compression optimization",
			});
		}
		...
		if (avgCompressionRatio > 40) {
			recommendations.push({
				...
				suggestion:
					"Enable Brotli compression and optimize compressible assets",
			});
		}
	...
	printSummary(analysis) {
		...
		console.log(
			`📊 Static Data: ${this.formatBytes(analysis.staticData.totalSize)}`,
		);

		console.log("\n📋 By File Type:");
		for (const [type, data] of Object.entries(analysis.totals.byType)) {
			console.log(
				`  ${type}: ${data.count} files, ${this.formatBytes(data.size)}`,
			);
		}
