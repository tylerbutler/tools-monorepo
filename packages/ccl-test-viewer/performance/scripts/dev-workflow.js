...
class DevWorkflow {
  ...
  async runFullAudit() {
    ...
    await this.runPerformanceCheck({ quick: false, lighthouse: true, bundle: true });
  ...
  async compareBranches(baseBranch = 'main', targetBranch = 'HEAD') {
    ...
    try {
      ...
      execSync(`git checkout ${currentBranch}`, { cwd: projectRoot, stdio: 'pipe' });
  ...
  compareResults(base, target, baseBranch, targetBranch) {
    ...
    if (base.lighthouse && target.lighthouse) {
      console.log('\n🔍 Lighthouse Scores:');
      this.compareMetric('Performance', base.lighthouse.performance, target.lighthouse.performance, '%');
      this.compareMetric('Accessibility', base.lighthouse.accessibility, target.lighthouse.accessibility, '%');
      this.compareMetric('Best Practices', base.lighthouse.bestPractices, target.lighthouse.bestPractices, '%');
      this.compareMetric('SEO', base.lighthouse.seo, target.lighthouse.seo, '%');

      console.log('\n🚀 Core Web Vitals:');
      this.compareMetric('LCP', base.lighthouse.lcp, target.lighthouse.lcp, 'ms', true);
      this.compareMetric('CLS', base.lighthouse.cls, target.lighthouse.cls, '', true);
      this.compareMetric('FID', base.lighthouse.fid, target.lighthouse.fid, 'ms', true);
    }

    if (base.bundle && target.bundle) {
      console.log('\n📦 Bundle Analysis:');
      this.compareMetric('Total Size', base.bundle.size, target.bundle.size, 'B', true);
      this.compareMetric('Gzipped Size', base.bundle.gzipSize, target.bundle.gzipSize, 'B', true);
      this.compareMetric('File Count', base.bundle.count, target.bundle.count, '', true);
    }

    console.log('\n');
  }

  compareMetric(name, baseValue, targetValue, unit = '', lowerIsBetter = false) {
    ...
    const formattedBase = unit === 'B' ? this.formatBytes(baseValue) : baseValue.toFixed(unit === '' ? 0 : 1);
    const formattedTarget = unit === 'B' ? this.formatBytes(targetValue) : targetValue.toFixed(unit === '' ? 0 : 1);
    const formattedDiff = unit === 'B' ? this.formatBytes(Math.abs(diff)) : Math.abs(diff).toFixed(unit === '' ? 0 : 1);

    console.log(`${indicator} ${name.padEnd(20)} ${formattedBase} → ${formattedTarget} (${diff >= 0 ? '+' : '-'}${formattedDiff} / ${percentChange.toFixed(1)}%)`);
  }
