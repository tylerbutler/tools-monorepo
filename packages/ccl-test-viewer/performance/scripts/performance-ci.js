...
class PerformanceCI {
    ...
    if (existsSync(lighthousePath)) {
      ...
      if (links.length > 0) {
        ...
        if (existsSync(reportPath)) {
          ...
          this.currentResults.lighthouse = this.extractLighthouseMetrics(report);
        }
      }
    }
  ...
  async loadBaselineResults() {
    ...
    console.log(`📋 Comparing ${commitHash.slice(0, 7)} with baseline ${baseCommit.slice(0, 7)}`);

    // Load historical data for baseline comparison
    const historyPath = join(projectRoot, 'perf-data/lighthouse-reports/history.json');
    ...
    console.log('📊 Baseline results loaded:', Object.keys(this.baselineResults));
  }
  ...
  generateSummary() {
    ...
    const mediumRegressions = this.regressions.filter(r => r.severity === 'medium').length;
...
| FID | ${lighthouse.fid?.toFixed(0) || 'N/A'}ms | ${thresholds.fid.max}ms | ${lighthouse.fid <= thresholds.fid.max ? '✅' : '❌'} |`;
  ...
  async sendAlerts() {
    ...
    for (const regression of highRegressions) {
      console.log(`   ${regression.metric}: ${regression.current} (${regression.difference})`);
    }
  }
  ...
  getBaselineCommit() {
    try {
      ...
      return execSync('git merge-base HEAD origin/main', { encoding: 'utf8', cwd: projectRoot }).trim();
    } catch {
      // Fallback to previous commit
      try {
        return execSync('git rev-parse HEAD~1', { encoding: 'utf8', cwd: projectRoot }).trim();
      } catch {
        return 'unknown';
      }
    }
