...
class PerformanceDashboard {
    ...
    async loadLighthouseData(timeRange) {
        try {
            const response = await fetch(`../perf-data/lighthouse-reports/history.json`);
    ...
    updateBudgetStatus() {
        ...
        const html = budgetItems.map(item => `
        ...
        const recommendations = [
            {
                ...
                description: 'Consider implementing lazy loading for the 436KB test data files. Load categories on demand and implement search index chunking.'
            },
            {
                title: 'Enable Compression',
                description: 'Ensure Brotli or Gzip compression is enabled for all static assets. This can reduce transfer sizes by 60-80%.'
            },
            {
                title: 'Implement Service Worker',
                description: 'Cache static data files and application shell using a service worker to improve repeat visit performance.'
            },
            {
                title: 'Bundle Size Optimization',
                description: 'Analyze bundle for unused code and consider code splitting for non-critical features.'
            }
        ];
    ...
    updateCharts() {
        ...
        if (this.charts.webVitals) {
            ...
            this.charts.webVitals.data.labels = vitalsData.map(d => this.formatDate(d.timestamp));
            this.charts.webVitals.data.datasets[0].data = vitalsData.map(d => d.lcp);
            this.charts.webVitals.data.datasets[1].data = vitalsData.map(d => d.fid);
            this.charts.webVitals.data.datasets[2].data = vitalsData.map(d => d.cls * 100);
        ...
        if (this.charts.lighthouse) {
            ...
            this.charts.lighthouse.data.labels = lighthouseData.map(d => this.formatDate(d.timestamp));
            this.charts.lighthouse.data.datasets[0].data = lighthouseData.map(d => d.performance);
            this.charts.lighthouse.data.datasets[1].data = lighthouseData.map(d => d.accessibility);
            this.charts.lighthouse.data.datasets[2].data = lighthouseData.map(d => d.bestPractices);
            this.charts.lighthouse.data.datasets[3].data = lighthouseData.map(d => d.seo);
        ...
        if (this.charts.bundleSize) {
            ...
            this.charts.bundleSize.data.labels = bundleData.map(d => this.formatDate(d.timestamp));
            this.charts.bundleSize.data.datasets[0].data = bundleData.map(d => Math.round(d.totals.size / 1024));
            this.charts.bundleSize.data.datasets[1].data = bundleData.map(d => Math.round(d.totals.gzipSize / 1024));
