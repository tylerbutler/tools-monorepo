...
export function validateTestData(jsonData: any, filename: string): UploadValidationResult {
  ...
  if (!Array.isArray(jsonData)) {
    ...
    return {
      ...
      errors,
      warnings,
      stats: { testCount: 0, categoryCount: 0, functions: [], features: [], behaviors: [] }
    };
  }
  ...
  for (let i = 0; i < jsonData.length; i++) {
    ...
    if (!test.expected || typeof test.expected.count !== 'number') {
      errors.push(`Test at index ${i} missing or invalid 'expected.count' field`);
    }
...
export function jsonToTestCategory(jsonData: GeneratedTest[], filename: string): TestCategory {
  const categoryName = filename.replace(/\.json$/i, '').replace(/[-_]/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
